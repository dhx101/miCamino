const miCamino = {
	...
	objetivo: "recorrer los 14 ochomiles y llegar hasta la cima del mundo: el Everest.",
	material: [
		{
			nombre: "Piolets",
			descripcion:
...
for (let i = 0; i < miCamino.material.length; i++) {
	let equipo = miCamino.material[i];
	if (equipo.nombre == "Piolets") {
		console.log(`lo primero que debo llevar es ${equipo.nombre} y es ${equipo.descripcion}`);
	} else if (equipo.nombre == "Crampones") {
		console.log(`lo segundo que debo llevar es ${equipo.nombre} y es ${equipo.descripcion}`);
	} else if (equipo.nombre == "Cuerdas") {
		console.log(`lo tercero que debo  llevar es ${equipo.nombre} y es ${equipo.descripcion}`);
	} else if (equipo.nombre == "Mochila") {
		console.log(`y por ultimo debo llevar la ${equipo.nombre} que es ${equipo.nombre}`);
	}
}

//4º: vamos a decir: para subir al {nombre de la cima}  debo subir {altura } metros y debo viajar a {ubicacion}

for (let picos in miCamino.cimas) {
	let cima = miCamino.cimas[picos];
	console.log(
...
//5º: para llegar a la cima tendremos que darnos aliento y decirnos la frase que esta divida y que tiene que quedar así: "Tranquilo,lo peor ha pasado ya.¡Animo!
	...
	let cima = miCamino.subiendoAlEverest[0][0].campoBase[0]["campoI"]["campoII"]["campoIII"]["campoIV"];
	// console.log(cima)
	let frase1Array = [];
	for (let i = 0; i < cima.length; i++) {
		// console.log(typeof(cima[i]))
		if (typeof cima[i] == "string") {
			frase1Array.push(cima[i]);
			// console.log("es string")
		} else if (typeof cima[i] == "number") {
			frase1Array.push(cima[i]);
			// console.log("es num")
		}
	}
	console.log(frase1Array.join(" "));
	console.log(cima[cima.length - 1].cima);
...
for (let i = 0; i < miCamino.material.length; i++) {
	let equipo = miCamino.material[i];
	let newUrl = [];
	for (let i = 0; i < equipo.imagenes.length; i++) {
		let imgUrl = equipo.imagenes[i];

		if (imgUrl.imagen1 !== undefined) {
			// console.log(imgUrl.imagen1);
      newUrl.push(imgUrl.imagen1.url)
		}
    if (imgUrl.imagen2 !== undefined) {
			// console.log(imgUrl.imagen2);
      newUrl.push(imgUrl.imagen2.url)
		}
	}
	console.log(newUrl);
}
